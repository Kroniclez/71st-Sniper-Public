...
killedPlayer( killId, victim, weapon, meansOfDeath )
{
	...
	//self thread updateCurrentCombo();
...
updateCurrentCombo() // Multi kill Combo system
{
	self endon ( "disconnect" );
	level endon ( "game_ended" );
	...
	if ( self.recentCombo > 1 )
		self.ComboScore = Int(( 100 * self.recentCombo )); // Adds 2 extra kills per Double. Example: Double kill now counts as 4 kills, triple counts as 6 etc.
		//self.killcombo = Int(( 2 * self.recentCombo ));
		//self.kills += self.killcombo;
