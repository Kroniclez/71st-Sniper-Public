...
doFinalKillCamFX( camTime )
{
	...
	if ( intoSlowMoTime > 1.0 ) // Time when killcam starts slowmo
	{
		intoSlowMoTime = 1.0; // Timer for slowmo to start/end on steam MW2?
		wait( camTime - 1.0 ); 
	}
	
	
	//Steam killcam.gsc slowmo settings 
	//Causes lag on Dedicated servers, fine in Demo playback & private matchs -- 
	
	setSlowMotion( 1.0, 0.30, 0 );
	wait( intoSlowMoTime + .5 );
	setSlowMotion( 0.30, 1, 0 );
