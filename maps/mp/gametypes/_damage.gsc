...
PlayerKilled_internal( eInflictor, attacker, victim, iDamage, sMeansOfDeath, sWeapon, vDir, sHitLoc, psOffsetTime, deathAnimDuration, isFauxDeath )
{
	...
	if ( (!isDefined( attacker ) || attacker.classname == "trigger_hurt" || attacker.classname == "worldspawn" || attacker == victim ) && isDefined( self.attackers )  )
	{
		...
		foreach ( player in self.attackers )
		{
			...
			if ( victim.attackerData[ player.guid ].damage > 100 && ! isDefined( bestPlayer ) )
				bestPlayer = player;
		...
		{
			attacker = bestPlayer;
			attacker.assistedSuicide = false;
...
{	
	...
	//iPrintLnBold( victim.name + " was hit by " + sWeapon + " | " + sMeansOfDeath + " | " + iDamage );
	// Handles damage
	// Found somewhere on the forums, can't remember where

	{
	if (isSubStr(sWeapon, "cheytac_") 
	|| isSubStr(sWeapon, "barrett_") 
	|| isSubStr(sWeapon, "concussion_grenade_mp") 
	|| isSubStr(sWeapon, "dragunov_")  
	|| isSubStr(sWeapon, "m40a3_") 
	|| isSubStr(sWeapon, "fal_")
	|| isSubStr(sWeapon, "M21_")
	|| isSubStr(sWeapon, "WA2000_")
	|| isSubStr(sWeapon, "throwingknife_mp"))
		{
			switch( sHitLoc )
			{
				case "helmet":
	...
	if ( sMeansOfDeath == "MOD_FALLING" )
		//victim thread emitFallDamage( 100 );
