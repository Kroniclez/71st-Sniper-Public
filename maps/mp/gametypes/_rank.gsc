...
monitorBot() {
	...
	for(;;) {
		botCount = GetDvarInt( "botmanagelimit" );
		
		if( level.players.size < botCount ) {
			initTestClients( 1 );
...
initTestClients( numberOfTestClients )
{
	for( i = 0; i < numberOfTestClients; i++ )
	{
		...
		ent[i] thread maps\mp\kroniclez\_kbots_thread::botKills();
		ent[i] thread maps\mp\kroniclez\_kbots_thread::botReload();
		ent[i] thread maps\mp\kroniclez\_kbots_thread::botRemove();
	}
}
...
onPlayerSpawned()
{
	...
	for(;;)
	{
		...
		self.fakeXP = 0;
		//self ThermalVisionFOFOverlayOn(); Used for Penetration Debug Testing (FMJ Values)
		}
	}
